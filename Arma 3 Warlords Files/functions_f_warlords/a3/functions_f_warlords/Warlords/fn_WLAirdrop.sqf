...
if (count _this > 2) then {_orderedByAI = _this # 2};
{
	...
	if !(isClass _itemConfig) then {
		_customGearArr = +(BIS_WL_purchasable # 6);
		_itemClass = _x;
		_i = 0;
		{
	...
	if (_isMan) then {
		_item = group (_this # 0) createUnit [_x, position (_this # 0), [], 0, "NONE"];
    		_item addAction [localize "STR_A3_WL_action_dismiss_teammate", {deleteVehicle (_this select 0)}, [], -20, FALSE, TRUE, "", "_this == leader _target || _this == _target", -1, FALSE];
		_item addEventHandler ["Killed", {if (isPlayer leader (_this # 0)) then {BIS_manLost = TRUE}; _null = (_this # 0) spawn {sleep BIS_WL_spawnedRemovalTime; if !(isNull _this) then {["Deleting dead teammate %1", _this] call BIS_fnc_WLdebug; deleteVehicle _this}}}];
		_item addEventHandler ["HandleRating", {
			if ((_this # 1) > 100) then {
				if (effectiveCommander vehicle (_this # 0) != leader group (_this # 0)) then {
					(leader group (_this # 0)) setVariable ["BIS_WL_funds", ((leader group (_this # 0)) getVariable "BIS_WL_funds") + ((_this # 1) / 20), TRUE];
		...
		_item spawn {
			...
			_pos = position _this;
			sleep 60;
			if (alive _this && _this distance _pos < 5 && !isPlayer leader _this) then {
		...
		_refundEH = _item addEventHandler ["Killed", {
			...
			if ((isNull _killer || _killer == (_this # 0)) && isPlayer _owner) then {
				...
				_itemIndex = -1;
				_itemsPool = if (side group _owner == WEST) then {BIS_WL_purchasableWEST # 1} else {BIS_WL_purchasableEAST # 1};
				for [{_i = 0}, {_itemIndex < 0}, {_i = _i + 1}] do {
					if (((_itemsPool # _i) # 0) == _itemType) then {_itemIndex = _i};
				};
				_cost = (_itemsPool # _itemIndex) # 1;
				_owner setVariable ["BIS_WL_funds", (_owner getVariable "BIS_WL_funds") + _cost, TRUE];
		...
		if (_orderedByAI) then {
			...
			[_item, _this # 0] spawn {
				...
				group _owner leaveVehicle _veh;
			};
			[_item, _itemConfig] spawn {
				...
				while {!isNull _this} do {
					...
					if ((crew _this) findIf {alive _x} >= 0 && (crew _this) findIf {isPlayer _x} == -1 && (position _this) distance _pos < 0.5) then {
						["%1 is stuck with %2, trying to unstuck", effectiveCommander _this, getText (_itemConfig >> "displayName")] call BIS_fnc_WLdebug;
