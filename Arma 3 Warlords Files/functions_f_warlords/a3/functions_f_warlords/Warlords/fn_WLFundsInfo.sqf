...
while {TRUE} do {
	...
	_curFunds = floor (player getVariable "BIS_WL_funds");
	_osd_cp_current ctrlSetStructuredText parseText format ["<t shadow = '2' size = '%3' color = '%4'>%1 %2</t>", _curFunds, localize "STR_A3_WL_unit_cp", (1 call BIS_fnc_WLSubroutine_purchaseMenuGetUIScale), if (_curFunds <= _prevFunds) then {"#ffffff"} else {"#287e3d"}];
		...
		if !(BIS_WL_seizingBar_progress isEqualTo BIS_WL_seizingBar_progress_prev) then {
			...
			if (_cond) then {
				...
				BIS_WL_seizingBar_progress_loop = [_osd_cp_action_title, _osd_cp_progress_background, _osd_cp_progress] spawn {
					...
					_progressStartTime = serverTime - _timeout + _timeLeft;
					_progressEndTime = _progressStartTime + _timeout;
					_hidden = FALSE;
					while {TRUE} do {
						_progressCur = linearConversion [_progressStartTime,_progressEndTime,serverTime,0,1];
