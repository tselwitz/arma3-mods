...
if (_toSectorInConflict) then {
	_pos = [position _currentSector, 200 + ((_currentSector getVariable "Size") / 2), [_currentSector, player] call BIS_fnc_dirTo] call BIS_fnc_relPos;
...
if (!visibleMap || !alive player || lifeState player == "INCAPACITATED" || typeName (player getVariable "BIS_WL_FTPos") == typeName grpNull) exitWith {
	...
	if (_toSectorInConflict) then {
		player setVariable ["BIS_WL_funds", (player getVariable "BIS_WL_funds") + BIS_WL_FTCost, TRUE];
		{
