...
switch (_this) do {
	case "open": {
		...
		
		{
			...
			if (count (BIS_WL_purchasable # _id) > 0) then {
		...
		_purchase_request ctrlAddEventHandler ["ButtonClick", {
			if (uiNamespace getVariable ["BIS_WL_purchaseMenuItemAffordable", FALSE]) then {
				...
				switch (_class) do {
					...
					default {[_class, _cost, _category, _requirements] call BIS_fnc_WLrequestPurchase};
				};
			} else {
				playSound "AddItemFailed";
			};
		...
		_purchase_remove_item ctrlAddEventHandler ["ButtonClick", {
			...
			if (ctrlEnabled (_display displayCtrl 107)) then {
				...
				if (_refund > 0) then {
					...
					player setVariable ["BIS_WL_funds", (player getVariable "BIS_WL_funds") + _refund, TRUE];
		...
		_purchase_remove_all ctrlAddEventHandler ["ButtonClick", {
			...
			if (ctrlEnabled (_display displayCtrl 107)) then {
				...
				player setVariable ["BIS_WL_funds", (player getVariable "BIS_WL_funds") + _refundTotal, TRUE];
		...
		_purchase_transfer_ok ctrlAddEventHandler ["ButtonClick", {
			if (uiNamespace getVariable ["BIS_WL_fundsTransferPossible", FALSE]) then {
				...
				_amount = call compile ctrlText (_display displayCtrl 117);
				_targetArr = BIS_WL_allWarlords select {name _x == _targetName};
				if (count _targetArr > 0) then {
					playSound "AddItemOK";
					_target = _targetArr # 0;
					player setVariable ["BIS_WL_funds", (player getVariable "BIS_WL_funds") - _amount, TRUE];
					_target setVariable ["BIS_WL_funds", (_target getVariable "BIS_WL_funds") + _amount, TRUE];
