...
if !(_byAI || _vehClass == "static") then {
...
if !(_vehClass in ["ammo", "static", "autonomous"]) then {
	...
	if (_vehClass == "ammo") then {
		_veh spawn {
			sleep 300;
			deleteMarker (_this getVariable "BIS_WL_trackMarker");
			_this setVariable ["BIS_WL_trackMarker", ""];
		};
	};
	if (_vehClass == "autonomous") then {
		_veh spawn {
			sleep 60;
			deleteMarker (_this getVariable "BIS_WL_trackMarker");
			_this setVariable ["BIS_WL_trackMarker", ""];
...
while {!isNull _veh} do {
	...
	if ((crew _veh) findIf {alive _x} == -1) then {
		...
		if (time > (_veh getVariable "BIS_WL_deleteTimeout")) exitWith {deleteMarkerLocal _mrkrName; ["Deleting abandoned vehicle %1", _veh] call BIS_fnc_WLdebug; deleteVehicle _veh};
	} else {
		_tmoutUpdated = FALSE;
	};
	sleep 5;
