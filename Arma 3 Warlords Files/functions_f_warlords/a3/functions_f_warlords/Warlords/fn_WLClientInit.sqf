...
{
	_x addAction [localize "STR_A3_WL_action_dismiss_teammate", {deleteVehicle (_this select 0)}, [], -20, FALSE, TRUE, "", "_this == leader _target || _this == _target", -1, FALSE];
...
if (BIS_WL_playersAlpha > 0) then {
	_null = [] spawn {
		...
		while {TRUE} do {
			_tempArray = +BIS_WL_allWarlords select {isPlayer _x && _x != player && side group _x == side group player};
...
[] spawn {
	...
	_t = serverTime + 10;
	waitUntil {serverTime > _t || visibleMap};
	if !(visibleMap) then {
...
[] spawn {
	_currentSectorVarID = format ["BIS_WL_currentSector_%1", side group player];
	_t = serverTime + 10;
	waitUntil {serverTime > _t && !isNull (missionNamespace getVariable _currentSectorVarID)};
	sleep 5;
...
[] spawn {
	while {TRUE} do {
		...
		while {serverTime < _end} do {
			titleText [format ["%1\n\n%2", toUpper localize "str_a3_mission_failed_friendly_fire", ceil (_end - serverTime)], "PLAIN"];
