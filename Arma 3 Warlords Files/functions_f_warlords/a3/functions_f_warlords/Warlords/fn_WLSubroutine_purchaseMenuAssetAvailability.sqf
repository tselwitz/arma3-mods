...
if (_ret) then {
	switch (_class) do {
		...
		case "VotingReset": {
			...
			if (serverTime < ((missionNamespace getVariable [_sectorSelectedTimestampVarID, 0]) + BIS_WL_votingResetTimeout)) exitWith {_ret = FALSE; _tooltip = localize "STR_A3_WL_menu_resetvoting_restr1"};
		};
		case "Arsenal": {
			_owned = BIS_WL_sectorsArrayFriendly # 0;
			if (_owned findIf {[player, _x, TRUE] call BIS_fnc_WLInSectorArea} == -1) then {
